...
mod tests {
    ...
    #[test]
    fn intersect_monkey() -> Result<(), Box<dyn Error>> {
        // Test that all the same points of the monkey model are intersected by both meshes

        let monkey_model = Arc::new(MeshData::load_obj("assets/monkey.obj")?);
        let width = 2.75;
        let height = 2.0;

        let kd_monkey_mesh = KDMesh::new(&monkey_model, Shading::Flat);
        let kd_monkey_mesh_smooth = KDMesh::new(&monkey_model, Shading::Smooth);
        let monkey_mesh = Mesh::new(monkey_model.clone(), Shading::Flat);
        let monkey_mesh_smooth = Mesh::new(monkey_model, Shading::Smooth);
        ...
        (0..n).into_par_iter().zip((0..n).into_par_iter()).for_each(|(i, j)| {
            ...
            let origin = Vec3 {x, y, z: 3.0};

            let ray = Ray::new(origin, Vec3::forward_rh());
            let t_range = Range {start: EPSILON, end: INFINITY};

            assert_eq!(kd_monkey_mesh.ray_hit(&ray, &t_range), monkey_mesh.ray_hit(&ray, &t_range));
            assert_eq!(kd_monkey_mesh_smooth.ray_hit(&ray, &t_range), monkey_mesh_smooth.ray_hit(&ray, &t_range));
        });
